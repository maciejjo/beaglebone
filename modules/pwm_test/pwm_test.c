		...
		struct device_attribute *attr, char *buf)
{
	...
	pwm_test->duty = pwm_test->pwm->duty_cycle;
...
static DEVICE_ATTR(duty, S_IRUSR | S_IWUSR, pwm_test_show_duty,
		pwm_test_store_duty);
static DEVICE_ATTR(period, S_IRUSR | S_IWUSR, pwm_test_show_period,
		pwm_test_store_period);
static DEVICE_ATTR(polarity, S_IRUSR | S_IWUSR, pwm_test_show_polarity,
		pwm_test_store_polarity);
static DEVICE_ATTR(run, S_IRUSR | S_IWUSR, pwm_test_show_run,
		pwm_test_store_run);
...
static int pwm_test_probe(struct platform_device *pdev)
{
	...
	dev_err(dev, "Configure pwm. duty: %d, period: %d\n",
			pwm_test->duty, pwm_test->period);
